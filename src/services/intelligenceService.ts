...
2. 摘錄內容：針對每個關鍵論點，先引用原始網頁中的一段話。
3. 標註連結：在引用後方提供完整的 [日期＋媒體名稱](真實網址) 的超連結。請確保連結為當下可點擊且直接連往該資訊頁面的網址。
4. 最終校核：在輸出前，請再次檢查該連結是否存在於你的檢索結果中，若不確定連結的真實性，請直接註明【資料不足，無法確認】，嚴禁拼湊網址。
